...
export class UsersService {
  ...
  async createUser(createUserDto: CreateUserRequestDto): Promise<User> {
    ...
    const { salt, hash } = await this.passwordHashHelper.createPasswordHash(
      createUserDto.password,
    );

    // Create user with salt
    const userData = UserTransformer.createRequestDtoToEntity(
      createUserDto,
      salt,
    );
    ...
    // Auto-create playground workspace for new user
    try {
      await this.workspaceProvider.createPlaygroundWorkspace(
        {
          name: `${createUserDto.firstName}'s Playground`,
          description:
            'Personal workspace for database exploration and experimentation',
        },
        user.id,
      );
      ...
      console.error(
        `Failed to create playground workspace for user ${user.id}:`,
