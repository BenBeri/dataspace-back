...
export class WorkspaceService {
  ...
  async createWorkspace(
    ...
    const workspaceData = WorkspaceTransformer.createRequestDtoToEntity(
      createWorkspaceDto,
      ownerUserId,
      uniqueNameKey,
    );
    ...
    this.logger.log(
      `Successfully created workspace ${workspace.id} with key ${uniqueNameKey}`,
    );
    return workspace;
  }

  /**
   * Creates a playground workspace specifically for a user
   * @param createWorkspaceDto The workspace creation data
   * @param ownerUserId The ID of the user creating the workspace
   * @returns The created playground workspace
   */
  async createPlaygroundWorkspace(
    ...
    // Create workspace data with the unique key and set as playground type
    const workspaceData = WorkspaceTransformer.createRequestDtoToEntity(
      createWorkspaceDto,
      ownerUserId,
      uniqueNameKey,
    );
    workspaceData.workspaceType = WorkspaceType.PLAYGROUND;

    const repository = this.transactionManager.getRepository(Workspace);
    const workspace = await repository.save(workspaceData);

    this.logger.log(
      `Successfully created playground workspace ${workspace.id} with key ${uniqueNameKey}`,
    );
    return workspace;
  }
  ...
  async updateWorkspaceKmsKey(
    ...
    this.logger.log(
      `Updated workspace ${workspaceId} with KMS key ${kmsKeyId}`,
    );
  }
  ...
  async updateWorkspace(
    ...
    const updateData =
      WorkspaceTransformer.updateRequestDtoToEntity(updateWorkspaceDto);
